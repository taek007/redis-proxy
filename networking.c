...
int _addReplyToBuffer(redisClient *c, char *s, size_t len) {
	...
	if (len > available) {
		return REDIS_ERR; // 必须要有充足的空间
	}

	memcpy(c->buf + c->bufpos, s, len);
	c->bufpos += len;
	printf("_addReplyToBuffer %d\n",c->bufpos);
	return REDIS_OK;
...
//	if (sdsEncodedObject(obj)) 
		...
		char *str = obj->ptr;
	//	if (_addReplyToBuffer(c, obj->ptr, sdslen(obj->ptr)) != REDIS_OK) {
		if (_addReplyToBuffer(c, obj->ptr, strlen(str)) != REDIS_OK) {
			// 如果c->buf中的空间不够,就复制到c->reply链表中
			_addReplyObjectToList(c, obj);
		}else{
			printf("failue\n");
...
void processInputBuffer(redisClient *c) {
	...
	while (sdslen(c->querybuf)) {
		...
		//	aeCreateFileEvent(server.el, server.sev_pools ->sev[0].fd, AE_WRITABLE, sendReplyToServer, server.sev_pools ->sev[0])
