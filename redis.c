...
void createSharedObjects(void) {
	...
	shared.ok = createObject(REDIS_STRING, sdsnew("+OK 123\r\n"));
...
void initServerConfig() {
	...
	//server.server_pools->server[0].fd=10;

	//printf("DDDDD%d\n", server.server_pools->server[0].fd);
}
...
void initRealServer() {
	server.sev_pools  = (server_pools*) malloc(sizeof(server_pools));
	server.sev_pools ->size= 1;
	//server.sev_pools ->sev = (redisClient*) malloc(sizeof(redisClient)*server.sev_pools->size);
	 
	int i;
	
	//for(i = 0; i < server.sev_pools ->size; i++) {
	for(i = 0; i < 1; i++) {
           //	 if(server.sev_pools->sev[i].fd <= 0) 
				{
					server.sev_pools ->sev[i] = (redisClient*) malloc(sizeof(redisClient)*server.sev_pools->size);
				 struct sockaddr_in addr;
				  int yes = 1;
					...
					printf("initRealServer %d\n", sockfd1);
					if(fcntl(sockfd1, F_SETFD, O_NONBLOCK) < 0) {
						printf("333\n");
						close(sockfd1);
						return;
					}

	//					 if(setsockopt(sockfd1, SOL_SOCKET, SO_KEEPALIVE, &yes, sizeof(yes)) < 0) {
	//						close(sockfd1);
	//						return;
	//					}
		//			server.sev_pools ->sev[i].fd = sockfd1;
					...
					if(connect(sockfd1, (const struct sockaddr *)&addr, sizeof(addr))) {
						printf("222222 %s\n" ,strerror(errno));
					...
					server.sev_pools ->sev[i] = createServer(sockfd1);
					printf("现在只有一个real server sock fd is %d\n", server.sev_pools ->sev[i]->fd);
//					if (aeCreateFileEvent(server.el, server.sev_pools ->sev[i].fd, AE_READABLE, readQueryFromServer, NULL) == AE_ERR) {
//						return;
//					}
				
            }
	}
	printf("连接real redis 的个数为 %d\n", server.sev_pools->size);
